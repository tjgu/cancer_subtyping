...
Modified further by Tongjun Gu
...
def train_model(train_set_x_org=None, training_epochs=1000, batch_size=100,
                n_hidden=10, learning_rate=0.1, corruption_level=0.1, W=None, bhid=None, bvis=None, 
...
#    updates=[(learning_rate_shared,learning_rate_init*decay_rate**(epoch_variable//100))]) # thenao does not support math.pow, instead use T.pow() or a**b
    ...
    da=dA_finetuning(numpy_rng=rng, theano_rng=theano_rng, input=x, n_visible=n_visible, n_hidden=n_hidden, W=W, bhid=bhid, bvis=bvis)
    # get the formula of the cost and updates    
    cost,updates=da.get_cost_updates(corruption_level=corruption_level, learning_rate=learning_rate, cost_measure=cost_measure) # cost_measure can be either"cross_entropy" or "euclidean"
